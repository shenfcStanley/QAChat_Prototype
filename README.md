...
#### 4. The `test_eval.py` script in the `tests` directory is used to generate answers for 16 tested queries `queries.txt` stored under `tests/testdata`. Test results are under `tests/testresults`
<pre>python test_eval.py</pre>
...
- Acceptable Answer Rate (LLM Score ≥ 4): 81.3%
